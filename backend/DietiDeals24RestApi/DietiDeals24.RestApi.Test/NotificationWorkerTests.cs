...
public class NotificationWorkerTests
{
    ...
    public class AddNotificationTokenAsyncTests // Black e White Box
    {
        ...
        public AddNotificationTokenAsyncTests() // Black Box test
        {
            ...
            mockNotificationClient.Setup(client => client.CreatePlatformEndpointAsync(It.IsAny<CreatePlatformEndpointRequest>(), It.IsAny<CancellationToken>()))
                .ReturnsAsync(new CreatePlatformEndpointResponse { EndpointArn = "arn:aws:sns:..." });
        ...
        public async Task AddNotificationTokenAsync_TestWithValidInput()
        {
            ...
            var deviceToken = "359015081997370";
        ...
        public async Task AddNotificationTokenAsync_TestWithEmptyUserId()
        {
            ...
            // Act & Assert
            await Assert.ThrowsAsync<Exception>(() => _notificationWorker.AddNotificationTokenAsync(userId, deviceToken));
        }
    ...
    public class SendNotificationAsyncTests // Black e White Box test
    {
        ...
        public class BlackBoxTests
        {
            ...
            public async Task SendNotificationAsync_TestWithEmptyUserId_ValidNotificationDto()
            {
                ...
                //Act & Assert
                await Assert.ThrowsAsync<Exception>(() => _notificationWorker.SendNotificationAsync(userId, notificationDto));
        ...
        public class WhiteBoxTests
        {
           ...
           public async Task TestPathPublishFailsForOneEndpoint_1_2_3_5_6_7_8()
           {
               ...
               var exception = new Exception("Publish failed for arn:aws:sns:eu-west-1:123150819972:endpoint/APNS/ipad");
               ...
               _mockSnsClient
                   .Setup(s => s.PublishAsync(It.Is<PublishRequest>(p => p.TargetArn != "arn:aws:sns:eu-west-1:123150819972:endpoint/APNS/ipad"), default))
                   .ReturnsAsync(new PublishResponse());
               // Setup per "arn:aws:sns:eu-west-1:123150819972:endpoint/APNS/ipad"
               _mockSnsClient
                   .Setup(x => x.PublishAsync(It.Is<PublishRequest>(p => p.TargetArn == "arn:aws:sns:eu-west-1:123150819972:endpoint/APNS/ipad"), default))
                   .ThrowsAsync(exception);
               ...
               _mockLogger.Verify(
                   x => x.Log(
                       ...
                       It.IsAny<EventId>(),
                       It.IsAny<It.IsAnyType>(),
                       It.Is<Exception>(ex => ex.Message.Contains("Publish failed for arn:aws:sns:eu-west-1:123150819972:endpoint/APNS/ipad")),
                       It.IsAny<Func<It.IsAnyType, Exception, string>>()),
                   Times.Once);
               _mockNotificationService.Verify(s => s.AddNotificationAsync(notificationDto, userId), Times.Once);
               _mockNotificationService.Verify(s => s.GetEndPointArnFromUserIdAsync(userId), Times.Once);
               _mockSnsClient.Verify(x => x.PublishAsync(It.Is<PublishRequest>(p => p.TargetArn == "arn:aws:sns:eu-west-1:123150819972:endpoint/APNS/iphone" 
                   || p.TargetArn == "arn:aws:sns:eu-west-1:123150819972:endpoint/APNS/iphone2"), default), Times.Exactly(2));
               _mockSnsClient.Verify(x => x.PublishAsync(It.Is<PublishRequest>(p => p.TargetArn == "arn:aws:sns:eu-west-1:123150819972:endpoint/APNS/ipad"), default), Times.Once);
           }
        }
    }
}